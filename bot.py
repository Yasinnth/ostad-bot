...
RESULTS_CHANNEL_LINK = "https://t.me/teacherZBMU"
RESULTS_CHANNEL_HANDLE = "@teacherZBMU"
...
def skip_kb():
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="بعدی ⏭", callback_data="skip_specialty")]])
...
async def start(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "🧑‍🏫 نام و نام خانوادگی استاد رو بنویس:"
    )
...
async def get_professor(message: Message, state: FSMContext):
    ...
    await message.answer(
        "🎓 اگر تخصص یا مدرک استاد رو می‌دونی بنویس.\n"
        "اگر نمی‌دونی، روی دکمه‌ی زیر بزن:",
        reply_markup=skip_kb()
    )
    await state.set_state(Review.specialty)


@router.message(Review.specialty)
async def get_specialty_text(message: Message, state: FSMContext):
    await state.update_data(specialty=message.text)
    await message.answer(
        "📚 نام دانشکده‌ای که در آن هستید چیست؟",
        reply_markup=kb(["پزشکی", "داروسازی", "پرستاری", "مامایی", "➕ سایر"])
    )
    await state.set_state(Review.faculty)


@router.callback_query(Review.specialty, F.data == "skip_specialty")
async def skip_specialty(call: CallbackQuery, state: FSMContext):
    await state.update_data(specialty=None)
    await call.message.answer(
        "📚 نام دانشکده‌ای که در آن هستید چیست؟",
        reply_markup=kb(["پزشکی","بهداشت", "داروسازی","علوم آزمایشگاهی", "پرستاری", "مامایی", "➕ سایر"])
    )
    await state.set_state(Review.faculty)
    await call.answer()
...
async def get_faculty(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("📖 نام درس (یا درس‌هایی که با این استاد داشتید) رو بنویس:")
...
async def get_faculty_custom(message: Message, state: FSMContext):
    ...
    await message.answer("📖 نام درس (یا درس‌هایی که با این استاد داشتید) رو بنویس:")
    await state.set_state(Review.course)
...
async def score_knowledge(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("💬 پاسخگویی استاد به سوالاتتون رو از ۱ تا ۱۰ نمره بده:", reply_markup=score_kb())
...
async def get_contact(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        "✍️ حالا مهم‌ترین بخش: تجربه‌ای که بچه‌های ترم بعد باید بدونن رو بنویس.\n\n"
        "مثلاً:\n"
        "• برای امتحان باید تو طول ترم خوند یا شب امتحان جمع میشه؟\n"
        "• جزوه یا سوالات خاصی از ترم‌های قبل دست‌به‌دست میشه؟\n"
        "• نحوه‌ی طرح سوالات امتحان چطوره (تستی، تشریحی، از کجا میاد)؟\n"
        "• رفتار استاد با دانشجو، نکات و ریزه‌کاری‌های کلاس چیه؟\n\n"
...
async def get_comment(message: Message, state: FSMContext):
    ...
    specialty_line = data.get("specialty") or "نامشخص"

    summary = (
        f"👀 اطلاعات ثبت‌شده:\n\n"
        f"🧑‍🏫 استاد: {data['professor']}\n"
        f"🎓 تخصص/مدرک: {specialty_line}\n"
...
async def confirm(call: CallbackQuery, state: FSMContext):
    ...
    if call.data == "✅ ثبت نهایی":
        ...
        final_text = (
            ...
            "<b>یکجا جمع‌بندی می‌کنیم</b> و به‌صورت یک پست کامل منتشر می‌کنیم "
            "تا بقیه‌ی دانشجوها هم از تجربه‌ی شما استفاده کنن 📊🙌\n\n"
            f"📢 نتایج و جمع‌بندی نظرات تو این کانال گذاشته میشه:\n"
            f'👉 <a href="{RESULTS_CHANNEL_LINK}">{RESULTS_CHANNEL_HANDLE}</a>\n\n'
            f"📚 و اینم آیدی کانال اصلی:\n"
...
async def restart(call: CallbackQuery, state: FSMContext):
    await state.clear()
    await call.message.answer("🧑‍🏫 نام و نام خانوادگی استاد رو بنویس:")
    await state.set_state(Review.professor)
    await call.answer()
