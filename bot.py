...
async def get_professor(message: Message, state: FSMContext):
    ...
    await message.answer("📚 دانشکده یا گروه استاد چیه؟ (مثلاً فنی مهندسی، علوم پایه، حقوق و...)")
...
async def get_course(message: Message, state: FSMContext):
    ...
    await message.answer("🗓️ در چه ترمی با این استاد کلاس داشتی؟ (مثلاً مهر ۱۴۰۴)")
...
async def get_semester(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        reply_markup=kb(["جزوه", "PowerPoint/PDF", "کتاب مرجع", "جزوه‌نویسی در کلاس", "منبع مشخصی معرفی نمی‌کند"])
    )
...
async def confirm(call: CallbackQuery, state: FSMContext):
    ...
    if call.data == "✅ ثبت نهایی":
        ...
        await call.message.answer("✅ نظر شما با موفقیت ثبت شد.\n🙏 ممنون که تجربه‌ات رو به اشتراک گذاشتی.\nنظر شما بعد از بررسی منتشر می‌شود.")

        if ADMIN_ID:
            await bot.send_message(ADMIN_ID, f"🔔 نظر جدید ثبت شد:\n\n{data['professor']} - {data['course']}")
    else:
        await call.message.answer("❌ لغو شد. برای شروع دوباره /start رو بزن.")
