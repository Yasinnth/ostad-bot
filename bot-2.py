...
async def start(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "⚠️ <b>لطفاً اطلاعات دقیق و واقعی وارد کن.</b>\n"
        "این نظرات جمع‌بندی و توسط هوش مصنوعی تحلیل می‌شن تا خروجی درست و قابل‌استفاده برای بقیه دانشجوها تولید بشه. "
        "پس لطفاً از نوشتن مطالب بی‌ربط، شوخی یا اطلاعات نادرست خودداری کن و با دقت جواب بده.\n\n"
...
async def get_faculty(call: CallbackQuery, state: FSMContext):
    if call.data == "➕ سایر":
        await call.message.answer("✏️ اگر رشته‌ی شما در بین گزینه‌ها نبود، نام رشته‌تون رو بنویسید:")
        await call.answer()
        return
